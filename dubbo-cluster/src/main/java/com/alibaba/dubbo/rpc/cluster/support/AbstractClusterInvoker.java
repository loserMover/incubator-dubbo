...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
     ...
     * 粘滞连接用于有状态服务，尽可能让客户端总是向同一服务提供者发起调用，除非该提供者挂了，再连另外一台
    ...
    public boolean isAvailable() {
        //若有粘滞连接Invoker，基于它判断
        Invoker<T> invoker = stickyInvoker; //只想，避免并发
        if (invoker != null) {
     ...
     *
     * @param loadbalance load balance policy LoadBalance对象
     ...
     * @return 最终的Invoker对象
     * @throws RpcException 当发生RpcException异常
     */
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        boolean sticky = invokers.get(0).getUrl().getMethodParameter(methodName, Constants.CLUSTER_STICKY_KEY, Constants.DEFAULT_CLUSTER_STICKY);
        {
            ...
            //若stickyInvoker不存在Invokers中，说明不在候选中，需要置空，重新选择
            if (stickyInvoker != null && !invokers.contains(stickyInvoker)) {
                stickyInvoker = null;
            }
            //ignore concurrency problem
            //若开启粘滞连接的特性，且stickyInvoker不存在selected中，则返回stickInvoker这个Invoker对象
            if (sticky && stickyInvoker != null && (selected == null || !selected.contains(stickyInvoker))) {
                //若开启排除非可用的Invoker的特性，则校验stickyInvoker是否可用。若可用则进行返回
    ...
    private Invoker<T> doSelect(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        //【第二种】如果只有两个Invoker，且候选集合不为空。退化成轮询
        ...
        //【第三种】使用LoadBalance，选择一个Invoker对象
        Invoker<T> invoker = loadbalance.select(invokers, getUrl(), invocation);

        //If the `invoker` is in the  `selected` or invoker is unavailable && availablecheck is true, reselect.
        //如果selected中包含（优先判断）或者不可用且availablecheck=true则重试
