...
public class JavassistCompiler extends AbstractCompiler {
    /**
     * @desc 正则表达式 - 匹配 import
     */
    private static final Pattern IMPORT_PATTERN = Pattern.compile("import\\s+([\\w\\.\\*]+);\n");
    /**
     * @desc 正则表达式 - 匹配 extends
     */
    private static final Pattern EXTENDS_PATTERN = Pattern.compile("\\s+extends\\s+([\\w\\.]+)[^\\{]*\\{\n");
    /**
     * @desc 正则表达式 - 匹配 implements
     */
    ...
    public Class<?> doCompile(String name, String source) throws Throwable {
        ...
        while (matcher.find()) {
            ...
            if (pkg.endsWith(".*")) {//引用整个包下的类和接口
                ...
                if (pi > 0) {
                    ...
                    pool.importPackage(pkgName);
                    importPackages.add(pkgName);
                    fullNames.put(pkg.substring(pi + 1), pkg); //设置类名
                }
            }
        }
        ...
        if (matcher.find()) {
            ...
            for (String iface : ifaces) {
                iface = iface.trim();
                String ifaceClass;
                if (iface.contains(".")) { //内部的接口，例如‘implements A.B’
                    ifaceClass = iface;
        ...
        //匹配method。使用分割的方式，实际上，分割出的不仅仅有方法。
